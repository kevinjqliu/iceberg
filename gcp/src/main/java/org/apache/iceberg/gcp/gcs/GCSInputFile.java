...
class GCSInputFile extends BaseGCSFile implements InputFile {
  ...
  GCSInputFile(
    ...
    super(storage, prefixedStorage, blobId, gcpProperties, metrics);
  ...
  public SeekableInputStream newStream() {
    if (gcpProperties().isGcsAnalyticsCoreEnabled()) {
      try {
        return GcsAnalyticsCoreHelper.newInputStream(
            prefixedStorage().gcsFileSystem(), blobId(), blobSize, metrics());
      } catch (IOException e) {
        LOG.error(
